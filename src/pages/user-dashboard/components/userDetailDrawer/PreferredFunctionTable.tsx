import { PreferredFunctionTableProps } from '../../../../types/user-dashboard';

export default function PreferredFunctionTable({
  headers,
  rows,
  getRowKey,
  className,
  fullWidth,
}: PreferredFunctionTableProps) {
  return (
    <div
      className={`${
        fullWidth ? 'w-full max-w-5xl mx-auto' : 'inline-block'
      } rounded-xl overflow-hidden shadow-sm bg-white ${className ?? ''}`}>
      <table
        className={`${
