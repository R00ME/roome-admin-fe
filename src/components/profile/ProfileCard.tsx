...
export default function ProfileCard({ adminEmail, username }: AdminUser) {
  ...
  return (
    <Card className='fixed z-45 top-20 right-8 flex flex-col justify-center items-center w-58 rounded-xl p-6 shadow-lg gap-3 '>
      {/* 프로필 이미지 */}
        ...
        <div className='flex flex-col items-center'>
          <div className='flex gap-2'>
            <span className='flex items-center gap-1'>
              <p className={`text-[#293F66] font-semibold`}>
                {username.length > 4 ? username.slice(0, 5) + '...' : username}
              </p>
